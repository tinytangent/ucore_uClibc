...
rootfs/$(notdir $1): rootfs $(addprefix $(TEST_SRC_ROOT)/, $(addsuffix .o,$1))
	@echo LINK $$@
	$(LD) -static  -o $$@ -L$(LIB_DIR)/usr/lib -L$(LIB_DIR)/lib $(LIB_DIR)/usr/lib/crt1.o $(LIB_DIR)/usr/lib/crti.o  $(addprefix $(TEST_SRC_ROOT)/, $(addsuffix .o,$1)) -lpthread -lm -lrt -lc $(LIBGCC) $(LIB_DIR)/usr/lib/crtn.o
	$(STRIP) $$@
